...
class InverseOpParameterTyped : public ::testing::Test{
};

TYPED_TEST_SUITE(InverseOpParameterTyped, AllTypesT);

TYPED_TEST(InverseOpParameterTyped, Inverse_Operability_And_Correctness_Primitives)
{
    using T = TypeParam;
    ...
    if constexpr (op_allowed<Policy, T>) {
        ...
        auto par = methodverse::parameter::inverse(p1);
