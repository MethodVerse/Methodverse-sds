...
namespace methodverse::operatortest::coefw_div {

// ---------- Typed test suite over all pairs ----------
template <class PairT>
class CoefwDivOpParameterTyped : public ::testing::Test{
};

TYPED_TEST_SUITE(CoefwDivOpParameterTyped, AllPairsT);
TYPED_TEST_SUITE(CoefwDivOpParameterTyped, AllPairsT);

TYPED_TEST(CoefwDivOpParameterTyped, CoefwDiv_Operability_And_Correctness_Primitives)
{
    ...
    using Policy = op_policy<CL, CR, coefw_div_op>;
    using RT = op_return_t<Policy, L, R>;
    if constexpr (op_allowed<Policy, L, R>) {
        auto pr = makeSample<L, R, RT>();
        auto actual = methodverse::parameter::coefw_div(pr.left, pr.right);               // test the operator to here
        ...
        auto par = methodverse::parameter::coefw_div(p1, p2); 
        ...
        par = methodverse::parameter::coefw_div(p1, p2);
        ExpectEqualSmart(actual, par[0]);
        ExpectEqualSmart(actual, par[1]);   
        ...
        EXPECT_THROW(par = methodverse::parameter::coefw_div(p1, p2), std::runtime_error);
    }
    else {
