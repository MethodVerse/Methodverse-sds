...
class CoefwMulOpTyped : public ::testing::Test{
};

TYPED_TEST_SUITE(CoefwMulOpTyped, AllPairsT);

TYPED_TEST(CoefwMulOpTyped, CoefwMul_Operability_And_Correctness_Primitives)
{
    ...
    if constexpr (op_allowed<Policy, L, R>) {
        ...
        auto actual2 = methodverse::parameter::coefw_mul(pr.left, pr.right);               // test the operator to here
        ...
        auto arr = methodverse::parameter::coefw_mul(larr, rarr);
        ...
        arr = methodverse::parameter::coefw_mul(larr, rarr);
        ExpectEqualSmart(pr.result, arr[0]);
        ExpectEqualSmart(pr.result, arr[1]);    
        ...
        EXPECT_THROW(arr = methodverse::parameter::coefw_mul(larr, rarr), std::runtime_error);
    }
    else {
