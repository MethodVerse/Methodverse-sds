...
namespace methodverse::operatortest::coefw_mul {

// ---------- Typed test suite over all pairs ----------
template <class PairT>
class CoefwMulOpParameterTyped : public ::testing::Test{
};

TYPED_TEST_SUITE(CoefwMulOpParameterTyped, AllPairsT);
TYPED_TEST_SUITE(CoefwMulOpParameterTyped, AllPairsT);

TYPED_TEST(CoefwMulOpParameterTyped, CoefwMul_Operability_And_Correctness_Primitives)
{
    ...
    using Policy = op_policy<CL, CR, coefw_mul_op>;
    using RT = op_return_t<Policy, L, R>;
    if constexpr (op_allowed<Policy, L, R>) {
        auto pr = makeSample<L, R, RT>();
        auto actual = methodverse::parameter::coefw_mul(pr.left, pr.right);               // test the operator to here
        ...
        auto par = methodverse::parameter::coefw_mul(p1, p2); 
        ...
        par = methodverse::parameter::coefw_mul(p1, p2);
        ExpectEqualSmart(actual, par[0]);
        ExpectEqualSmart(actual, par[1]);   
        ...
        EXPECT_THROW(par = methodverse::parameter::coefw_mul(p1, p2), std::runtime_error);
    }
    else {
