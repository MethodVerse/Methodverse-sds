...
class CoefwDivOpTyped : public ::testing::Test{
};

TYPED_TEST_SUITE(CoefwDivOpTyped, AllPairsT);

TYPED_TEST(CoefwDivOpTyped, CoefwDiv_Operability_And_Correctness_Primitives)
{
    ...
    if constexpr (op_allowed<Policy, L, R>) {
        ...
        auto actual2 = methodverse::parameter::coefw_div(pr.left, pr.right);               // test the operator to here
        ...
        auto arr = methodverse::parameter::coefw_div(larr, rarr);
        ...
        arr = methodverse::parameter::coefw_div(larr, rarr);
        ExpectEqualSmart(pr.result, arr[0]);
        ExpectEqualSmart(pr.result, arr[1]);    
        ...
        EXPECT_THROW(arr = methodverse::parameter::coefw_div(larr, rarr), std::runtime_error);
    }
    else {
